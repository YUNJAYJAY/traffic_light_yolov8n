...
# YOLOv5 모델 로드
model = torch.hub.load('ultralytics/yolov5', 'custom', path='/home/jay20_04/traffic_light/src/traffic_light/models/best.pt')  # YOLOv5 모델 경로
...
def image_callback(msg):
    try:
        ...
        # YOLOv5 모델로 예측 수행 (확대된 ROI)
        results = model(sharpened)

        # 바운딩 박스, 신뢰도, 클래스 정보 추출
        for det in results.xyxy[0].cpu().numpy():  # detections를 반복
